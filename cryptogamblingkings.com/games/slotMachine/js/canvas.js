...
function createBuyCryptoButton() {
    // Create a new shape (which will be our button)
    var button = new createjs.Shape();
    
    // Set the dimensions and color of the button
    button.graphics.beginFill("#f00").drawRoundRect(0, 0, 150, 60, 10);
    
    // Create a text element for the button's label
    var buttonText = new createjs.Text("Buy Crypto", "20px Arial", "#fff");
    
    // Center the text inside the button
    buttonText.x = button.getBounds().width / 2;
    buttonText.y = button.getBounds().height / 2;
    ...
    button.addEventListener("click", function(event) {
        // This function will be run when the button is clicked
        console.log("Buy Crypto button was clicked!");
        // You can replace the console.log with whatever you want to do when the button is clicked
    });
    
    // Add the button to the stage so it will be displayed
...
function initGameCanvas(w,h){
	...
	//Create the Buy Crypto button
	// createBuyCryptoButton()
}
...
function buildGameCanvas(){
	...
	creditContainer = new createjs.Container(); // CREDITS
	...
	// buttonBuyCrypto = new createjs.Bitmap(loader.getResult('buttonBuyCrypto'));
	// centerReg(buttonBuyCrypto);
	...
	optionsContainer.addChild(buttonFullscreen, buttonSoundOn, buttonSoundOff, buttonExit); //buttonExit
